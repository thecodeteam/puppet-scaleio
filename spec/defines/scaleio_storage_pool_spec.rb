...
#    :scanner_bandwidth_limit => '25',
  ...
  it 'present' do
    is_expected.to contain_scaleio__cmd('storage pool name present').with(
      :action => 'present',
      :entity => 'storage_pool',
      :value => 'name',
...
### Checks checksum_mode
  ...
  context 'with checksum_mode is enabled' do
    ...
    it 'sets checksum mode' do
      is_expected.to contain_scaleio__set('storage pool name set_checksum_mode').with(
        :is_defined => 'enable',
        :change => "--enable_checksum")
    end

    it { is_expected.to contain_scaleio__cmd('storage pool name set_checksum_mode').with(
...
### Checks zero_padding_policy
  ...
  context 'with zero_padding_policy is enabled' do
    ...
    it 'modifies zero_padding policy' do
      is_expected.to contain_scaleio__set('storage pool name modify_zero_padding_policy').with(
        :is_defined => 'enable',
        :change => "--enable_zero_padding")
    end

    it { is_expected.to contain_scaleio__cmd('storage pool name modify_zero_padding_policy').with(
...
### Checks rmcache_write_handling_mode
  ...
  context 'with rmcache_write_handling_mode is cached' do
    ...
    it 'sets rmcache_write_handling_mode' do
      is_expected.to contain_scaleio__set('storage pool name set_rmcache_write_handling_mode').with(
        :is_defined => 'cached',
        :change => "--rmcache_write_handling_mode cached --i_am_sure")
    end

    it { is_expected.to contain_scaleio__cmd('storage pool name set_rmcache_write_handling_mode').with(
...
### Checks rmcache_usage
  ...
  context 'with rmcache_usage is used' do
    ...
    it 'sets rmcache_usage' do
      is_expected.to contain_scaleio__set('storage pool name set_rmcache_usage').with(
        :is_defined => 'use',
        :change => "--use_rmcache --i_am_sure")
    end

    it { is_expected.to contain_scaleio__cmd('storage pool name set_rmcache_usage').with(
...
### Checks spare_policy
  ...
  context 'with spare_policy is defined' do
    ...
    it 'modifies spare policy' do
      is_expected.to contain_scaleio__set('storage pool name modify_spare_policy').with(
        :is_defined => '10',
        :change => "--spare_percentage 10 --i_am_sure")
    end

    it { is_expected.to contain_scaleio__cmd('storage pool name modify_spare_policy').with(
  ...
  context 'with spare_policy is undefined' do
    ...
    it { is_expected.not_to contain_scaleio__cmd('storage pool name modify_spare_policy')}
...
#  end
  ...
  context 'with scanner_mode is disabled' do
    ...
    it 'sets disable_background_device_scanner' do
      is_expected.to contain_scaleio__set('storage pool name disable_background_device_scanner').with(
        :is_defined => 'disable',
        :change => ' ')
    end

    it { is_expected.to contain_scaleio__cmd('storage pool name disable_background_device_scanner').with(
  ...
  context 'with scanner_mode is device_only' do
    ...
    it 'sets device_only_background_device_scanner' do
      is_expected.to contain_scaleio__set('storage pool name device_only_background_device_scanner').with(
        :is_defined => 'device_only',
        :change => " ")
    end

    it { is_expected.to contain_scaleio__cmd('storage pool name device_only_background_device_scanner').with(
  ...
  context 'with scanner_mode is data_comparison' do
    ...
    it 'sets data_comparision_background_device_scanner' do
      is_expected.to contain_scaleio__set('storage pool name data_comparison_background_device_scanner').with(
        :is_defined => 'data_comparison',
        :change => " ")
    end

    it { is_expected.to contain_scaleio__cmd('storage pool name data_comparison_background_device_scanner').with(
