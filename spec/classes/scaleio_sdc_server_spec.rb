...
describe 'scaleio::sdc_server' do
  ...
  it do
    is_expected.to contain_exec('scaleio repo public key').with(
  ...
  it 'contains scini_syncs' do
    is_expected.to contain_scaleio__sdc_server__scini_sync('repo_address').with_config(config)
    is_expected.to contain_scaleio__sdc_server__scini_sync('repo_user').with_config(config)
    is_expected.to contain_scaleio__sdc_server__scini_sync('repo_password').with_config(config)
    is_expected.to contain_scaleio__sdc_server__scini_sync('local_dir').with_config(config)
    is_expected.to contain_scaleio__sdc_server__scini_sync('module_sigcheck').with_config(config)
    is_expected.to contain_scaleio__sdc_server__scini_sync('repo_public_rsa_key').with_config(config)
    is_expected.to contain_scaleio__sdc_server__scini_sync('emc_public_gpg_key').with_config(config)
    is_expected.to contain_scaleio__sdc_server__scini_sync('sync_pattern').with_config(config)
  end
  it 'contains file_lines' do
    is_expected.to contain_file_line('scini_sync repo_address')
    is_expected.to contain_file_line('scini_sync repo_user')
    is_expected.to contain_file_line('scini_sync repo_password')
    is_expected.to contain_file_line('scini_sync local_dir')
    is_expected.to contain_file_line('scini_sync module_sigcheck')
    is_expected.to contain_file_line('scini_sync repo_public_rsa_key')
    is_expected.to contain_file_line('scini_sync emc_public_gpg_key')
    is_expected.to contain_file_line('scini_sync sync_pattern')
  end
  it do
